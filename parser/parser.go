...
// Using iota here helps us determine precedence of these constants
// Answers "does the * operator have higher precedence than ==?" etc.
const (
	_ int = iota
	LOWEST
	EQUALS
...
// Associate token types with their parsing precedence
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	// Get the appropriate prefix parsing function for the current token
...
func (p *Parser) parseReturnStatement() *ast.ReturnStatement {
	...
	// TODO: skipping the epxressions until we encounter a semicolon
	for !p.curTokenIs(token.SEMICOLON) {
		p.nextToken()
	}
...
// Peek the precedence of the next token. If it's not in the map of precedences,
